...
class ObjectDetector(object):
    def __init__(self,options=True,loadPath=None):

        #Check if SVM implementation is to be done with specific user-defined options
    ...
    #Prepare the annotations based on bounding box co-ordinates
    ...
    #Create a directory of all the images to be manually annotated
    ...
    #This creates the Histogram of Gradients. Used by train.py
    ...
    #Function to detect the object on un-annotated data. Used by test.py
    def detect(self,image,annotate=None):
        preds = self.predict(image)
        for (x,y,xb,yb) in preds:
            cv2.rectangle(image,(x,y),(xb,yb),(0,0,255),2)
            cv2.putText(image,annotate,(x,y),cv2.FONT_HERSHEY_PLAIN,1.0,(128,255,0),2)
